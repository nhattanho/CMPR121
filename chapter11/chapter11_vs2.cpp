...
public:
	...
	Chapter11(int newSize) : size(newSize)
	{
		array = new string[size];
	}
	Chapter11(const Chapter11& object) : size(object.size)
	{
		array = new string[size];
	...
	void deleteEntry(string entryToDelete)
	{
		if (size != 0)
		{
			...
			{
				for (int c = 0; c < positionToDelete; c++)
					newArray[c] = array[c];

				for (int c = positionToDelete + 1; c < size; c++)
	...
	void operator =(const Chapter11& object)
	{
		delete[] array;
		size = object.size;
		array = new string[size];
	...
	// Documentation: we don't handle with position out of range of size
	...
	// Documentation: we don't handle with position out of range of size
	void setItem(int position, string line)
	{
		if (size > 0)
		{
			if (position < size && position >= 0)
	...
	friend ostream& operator <<(ostream& outs, const Chapter11& object)
	{
		for (int c = 0; c < object.size; c++)
			outs << object.array[c] << endl;
		return outs;
	}
};
...
void Program_Project4()
{
	...
	StringVar object2(object1);
	cout << "\nobject2 after using copy constructor: " << endl;
	cout << object2;

	cout << "\nobject2 after using copyPiece function: " << endl;
	...
	cout << "\nTesting setChar function for objec2" << endl;
	object2.setChar(2, 'y');
	object2.setChar(4, 'z');
	cout << object2;

	cout << "\nTesting Onechar function for objec2" << endl;
	cout << "character at position 2: " << object2.Onechar(2) << endl;
	cout << "character at position 4: " << object2.Onechar(4) << endl;
...
void Program_Project11()
{
	...
	do
	{
		if (getBool("\nHi, do you want to add or delete member for object1?(A or D) ", 1))
		{
			do
			{
				name = getLine("Please enter a member's name: ");
				object1.addEntry(name);
			} while (getBool("Do you need to add more?(Y or N) ", 2));
			size = object1.currentSize();
			cout << "Size of object1: " << size << endl;
			cout << object1;
		}
		else
		{
	...
	Chapter11 object2;
	object2 = object1;
	cout << "\nobject2 after using assignment operator object2 = object1:" << endl;
	cout << object2;

	cout << "\nTesting for copy constructor, object3(object1)" << endl;
	cout << "Object3 after using the copy constructor: " << endl;
	Chapter11 object3(object1);
	cout << object3;

	cout << "\nTesting for setPosition of object3 at 5 and 2:" << endl;
	object3.setItem(5, "midnight is a due day!");//don't do anything because its position is out of range
	object3.setItem(2, "midnight is a due day!"); // array[2] will be change
	cout << object3;

	cout << "\nTesting for getPosition of object3 at 5 and 2:" << endl;
	cout << "At position 5: " << object3.getItem(5) << endl;//should be empty
	cout << "At position 2: " << object3.getItem(2) << endl;
}